      ...
      <section className="grid gap-8 lg:grid-cols-[1.02fr_0.98fr]">
            ...
            <div className="flex flex-col gap-5 border-b border-white/8 pb-6">
              ...
              <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
                <div className="max-w-2xl">
                  <p className="text-sm tracking-[0.22em] text-[#e3bb8d] uppercase">
                    Demo Configurator
                  </p>
                  <h1 className="mt-3 text-3xl font-semibold tracking-tight text-white sm:text-4xl">
                    Tạo trước phiên bản figure để khách cảm nhận được sản phẩm ngay.
                  </h1>
                  <p className="mt-4 max-w-2xl text-base leading-7 text-stone-300">
                    MVP này tập trung vào trải nghiệm chọn body, outfit, phụ kiện và gửi lead.
                    Phần đầu sẽ được custom riêng từ ảnh sau khi khách xác nhận đơn.
                  </p>
                </div>
                <div className="rounded-[28px] border border-[#e8c49a]/20 bg-[rgba(235,214,189,0.13)] px-5 py-4">
                  <p className="text-sm text-stone-200">Giá tham chiếu hiện tại</p>
                  <p className="mt-2 text-2xl font-semibold text-[#f5ddbc]">
                    {formatCurrency(activeSize.priceFrom)}
                  </p>
                  <p className="text-sm text-stone-300">Lead time {activeSize.productionTime}</p>
                </div>
              </div>
            </div>
            ...
            <form className="mt-8 space-y-8" onSubmit={handleSubmit}>
              ...
              <OptionCardGroup
                ...
                options={sizeOptions.map((size) => ({
                  id: size.id,
                  title: size.label,
                  description: `Chiều cao ${size.heightCm}cm, lead time ${size.productionTime}.`,
              ...
              <section className="space-y-4 rounded-[30px] border border-[#e3bb8d]/20 bg-[rgba(235,214,189,0.08)] p-5">
                <div>
                  <h3 className="text-sm font-semibold tracking-[0.18em] text-[#e3bb8d] uppercase">
                    Ảnh custom phần head
                  </h3>
                  <p className="mt-2 text-sm leading-6 text-stone-300">
                    Ở MVP này, ảnh chỉ dùng để tư vấn và custom phần đầu sau khi khách xác nhận
                    đơn. Chưa có xử lý AI hoặc render 3D head tự động.
                  </p>
                </div>

                <div className="grid gap-4 md:grid-cols-2">
                  <label className="space-y-2">
                    <span className="text-sm text-stone-300">Upload file ảnh</span>
                    <input
                      type="file"
                      accept="image/*"
                    ...
                    <p className="text-xs text-stone-500">
                      {imageFile ? `Đã chọn: ${imageFile.name}` : "Chưa upload file ảnh"}
                    </p>
                  </label>
              ...
              <section className="space-y-4 rounded-[30px] border border-white/10 bg-[rgba(255,255,255,0.04)] p-5">
                <div>
                  <h3 className="text-sm font-semibold tracking-[0.18em] text-stone-400 uppercase">
                    Gửi yêu cầu đặt hàng / nhận tư vấn
                  </h3>
                  <p className="mt-2 text-sm leading-6 text-stone-300">
                    Demo hiện tại sẽ lưu request qua API nội bộ. Nếu cấu hình Supabase, dữ liệu sẽ
                    được ghi sang database để bạn dùng ngay cho lead intake.
                  </p>
                </div>

                <div className="grid gap-4 md:grid-cols-2">
                  <label className="space-y-2">
                    <span className="text-sm text-stone-300">Tên khách</span>
                    <input
                      required
                      value={customerName}
                ...
                <label className="space-y-2">
                  ...
                  <textarea
                    ...
                    placeholder="Ví dụ: muốn in tên trên đế, cần giao trước ngày sinh nhật, làm concept giống cầu thủ yêu thích..."
                ...
                <div className="flex flex-col gap-4 border-t border-white/8 pt-5 md:flex-row md:items-center md:justify-between">
                  <div>
                    <p className="text-sm text-stone-300">Preview head đang ở chế độ mock.</p>
                    <p className="text-xs text-stone-500">
                      Sau này bạn chỉ cần thay asset 3D thật trong component preview.
                    </p>
                  </div>
                  <button
                    type="submit"
                  ...
                  >
                    {submitStatus.type === "loading" ? "Đang gửi..." : "Gửi yêu cầu ngay"}
                  </button>
                </div>
        ...
        <div className="space-y-6 lg:sticky lg:top-24 lg:self-start">
          ...
          <div className="rounded-[30px] border border-white/10 bg-[rgba(255,255,255,0.04)] p-5">
            <p className="text-xs tracking-[0.18em] text-stone-500 uppercase">MVP Architecture</p>
            <ul className="mt-4 space-y-3 text-sm leading-6 text-stone-300">
              <li>Preview render bằng primitive shape để giữ logic cấu hình và giao diện chạy ngay.</li>
              <li>Mỗi phần body, outfit, accessory, base được map qua object dữ liệu để dễ thay asset thật.</li>
              <li>API submit có fallback console nếu bạn chưa cấu hình Supabase ở giai đoạn đầu.</li>
            </ul>
          </div>
        </div>
      </section>
